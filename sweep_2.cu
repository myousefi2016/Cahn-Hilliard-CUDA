...
{
    string name = "./sweep_2/" + output + "_" + to_string(t) + ".vtk";
    ofstream ofile (name);
...
int main(int argc, char *argv[])
{
    ...
    while (D < D_f + deltaD)
      {
       while (gamma < gamma_f + deltaGamma)
        {
...
// compute result
    ...
    string name_c = "./sweep_2/integral_c_"+to_string(iterator)+".txt";
    ofstream ofile_c (name_c);

    string name_mu = "./sweep_2/integral_mu_"+to_string(iterator)+".txt";
    ofstream ofile_mu (name_mu);

    string name_f = "./sweep_2/integral_f_"+to_string(iterator)+".txt";
